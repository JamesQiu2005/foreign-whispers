...
def _synthesize_raw(tts_engine, text: str, wav_path: str, speaker_wav: str | None = None) -> bytes | None:
    ...
    If *speaker_wav* is provided and the engine supports voice cloning,
    forwards it as a kwarg.
    """
    ...
    try:
        ...
        # Engine doesn't accept speaker_wav (e.g. local Coqui fallback) — retry plain.
        try:
...
def text_file_to_speech(source_path, output_path, tts_engine=None, *,
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    for i, seg in enumerate(segments):
        ...
        # Per-segment voice resolution. ``voice_map`` (when present) maps
        # speaker labels → reference WAV. Falls back to the call-level
        # ``speaker_wav`` for segments without a known speaker.
        seg_voice = speaker_wav
        seg_speaker = seg.get("speaker")
        if voice_map:
            if seg_speaker and seg_speaker in voice_map:
                seg_voice = voice_map[seg_speaker]
            elif voice_map:
                # Round-robin across voices when speaker is unknown
                voices = list(voice_map.values())
                seg_voice = voices[i % len(voices)] if voices else seg_voice
    ...
    with tempfile.TemporaryDirectory() as synth_dir:
        def _do_synth(idx: int, text: str, sp_wav: str | None) -> tuple[int, bytes | None]:
            wav_path = str(pathlib.Path(synth_dir) / f"seg_{idx}.wav")
            return idx, _synthesize_raw(engine, text, wav_path, speaker_wav=sp_wav)
